...
const AddCourse = () => {
  ...
  const handleSubmit = async (e) => {
    try {
      ...
      // create a form data using the courseData that will be send using API
      const formData = new FormData()
      formData.append('courseData', JSON.stringify(courseData))
      formData.append('image', image)

      // now simply we have to make an API call
    ...
    <div className='h-screen overflow-scroll flex flex-col items-start justify-between md:p-8 md:pb-0 pt-8 pb-0'>
      <form onSubmit={handleSubmit} className='flex flex-col gap-4 max-w-md w-full text-gray-500'>
        <div className='flex flex-col gap-1'>
          <p>Course Title</p>
          <input onChange={(e)=> setCourseTitle(e.target.value)} type="text" value={courseTitle} placeholder='Enter Course Title' className='outline-none md:py-2.5 py-2 px-3 rounded border border-gray-500' required/>
        </div>
        ...
        <div>
          <p>Discount</p>
          <input onChange={e => setDiscount(e.target.value)} type="number" value={discount} min={0} max={100} placeholder='0' className='outline-none md:py-2.5 py-2 w-28 px-3 rounded border border-gray-500' required/>
        </div>
