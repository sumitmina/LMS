...
const CourseDetails = () => {
  ...
  const [openSection, setOpenSection] = useState({}) // for collapsing of lectures section in each chapter
    ...
    <div className='flex md:flex-row flex-col-reverse gap-10 relative items-start justify-between md:px-36 px-8 md:pt-30 pt-20 text-left'>
      <div className='absolute top-0 left-0 w-full h-section-height -z-1 bg-gradient-to-b from-cyan-100/70'></div>
        ...
        <div className='flex items-center space-x-2 pt-3 pb-1 text-sm'>
          ...
          <p className='text-blue-600'>{`(${courseData.courseRatings.length} ${courseData.courseRatings.length > 1 ? "ratigs" : "rating"})`}</p>

          <p>{courseData.enrolledStudents.length} {courseData.enrolledStudents.length > 1 ? "students" : "student"}</p>
        </div>

        <p className='text-sm'>Couse by <span className='text-blue-600 underline'>{courseData.educator.fullName}</span></p>
              ...
              <div className='pt-5'>
                {courseData.courseContent.map((chapter,index) => (
                  <div className='border border-gray-300 bg-white mb-2 rounded' key={index}>
                    ...
                    {/* displaying lectures in each chapters */}
        ...
        <div className='max-w-course-card z-10 shadow-custom-card rounded-t md:rounded-none overflow-hidden bg-white min-w-[300px] sm:min-w-[420px'>
          ...
          <div className='p-5'>
                ...
                <div className='pt-6'>
                  <p className='md:text-xl test-lg font-medium text-gray-800'>What's in the course?</p>
                  <ul className='ml-4 pt-2 text-sm md:text-default list-disc text-gray-500'>
                    <li>Lifetime access with free updates.</li>
                    <li>Step-by-step, hands-on project guidance.</li>
                    <li>Downloadble resources and source code.</li>
                    <li>Quizzes to test the knowledge.</li>
